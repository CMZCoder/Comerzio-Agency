            ...
            >
                <div className="services-track" ref={trackRef}>
                    {loopItems.map((service, index) => (
                        <div key={`${service.titleKey}-${index}`} className={`service-slide${service.logo ? ' has-logo' : ''}`}>
                            <div className="service-glow" />
                            {service.logo && (
                                service.titleKey === 'service_openclaw' ? (
                                    <OpenClawCharacter className="service-logo" />
                                ) : (
                                    <img src={service.logo} alt="" className="service-logo" />
                                )
                            )}
    ...
    return (
        ...
        >
            ...
            <div className="ai-slider-nav">
                <button ref={setPrevEl} className="ai-slider-nav__item ai-slider-nav__item_prev" type="button" aria-label={t('ai_prev')}>
                    <ChevronLeft size={18} />
                </button>
                <button ref={setNextEl} className="ai-slider-nav__item ai-slider-nav__item_next" type="button" aria-label={t('ai_next')}>
                    <ChevronRight size={18} />
                </button>
            </div>
            <Swiper
            ...
            <div ref={setPaginationEl} className="ai-slider-pagination" />
        </div>
    );
};
...
const AgencySections = () => {
    ...
    const services = useMemo(() => [
        ...
        { titleKey: 'service_aws', descriptionKey: 'service_aws_desc', logo: '/aws-color.png' },
        { titleKey: 'service_kubernetes', descriptionKey: 'service_kubernetes_desc' },
        { titleKey: 'service_devops', descriptionKey: 'service_devops_desc' },
        { titleKey: 'service_cloud', descriptionKey: 'service_cloud_desc' },
        { titleKey: 'service_openclaw', descriptionKey: 'service_openclaw_desc', logo: '/openclaw-color.png' }
    ], []);
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            (entries) => {
                ...
                // Hard check to prevent 'about' from triggering while on Hero
                if (mostVisible.target.id === 'about' && window.scrollY < 100) {
                    setActiveSection(null);
                    return;
                }
    ...
    useEffect(() => {
        if (!activeSection) {
            setTimeout(() => setRailAwake(false), 0);
            return;
        }
        setTimeout(() => wakeRail(), 0);
    }, [activeSection, wakeRail, setRailAwake]);
    ...
    useEffect(() => {
        ...
        const shapes = [
            ...
            compositeShape([
                { w: 40, fn: () => { // Volumetric Mane (Golden, shaggy)
                    const u=Math.random()*Math.PI*2;
